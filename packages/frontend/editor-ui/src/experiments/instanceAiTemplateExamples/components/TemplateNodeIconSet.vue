...
const allNodes = computed(() => (props.showAll ? props.nodes : filterTemplateNodes(props.nodes)));
const thirdPartyNodes = computed(() =>
	props.showAll ? allNodes.value : allNodes.value.filter((node) => !isNativeNode(node.name)),
);
...
<template>
	<div :class="$style.container">
		...
		>
			<NodeIcon :node-type="node" :size="20" :show-tooltip="true" />
		</div>
