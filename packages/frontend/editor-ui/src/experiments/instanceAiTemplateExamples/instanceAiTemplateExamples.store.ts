...
import type { ITemplatesCategory, ITemplatesWorkflow } from '@n8n/rest-api-client/api/templates';
import localData from './data/instanceAiTemplateExamples.json';
...
const typedLocalData = localData as unknown as LocalData;
...
// First four feature examples, always shown first before category selection
const FEATURED_DEFAULTS: LocalWorkflow[] = [
	{
		id: 900001,
...
const LOCAL_CATEGORIES: ITemplatesCategory[] = typedLocalData.categories.map((name, index) => ({
	id: index + 1,
	name,
}));
...
export const useInstanceAiTemplateExamplesStore = defineStore('instanceAiTemplateExamples', () => {
	...
	const workflows = ref<ITemplatesWorkflow[]>([]);
	...
	const subcategories = computed<string[]>(() => {
		const selectedCategory = LOCAL_CATEGORIES.find(
			(c) => String(c.id) === selectedCategoryId.value,
		);
		if (!selectedCategory) return [];
		const subcats = typedLocalData.subcategories[selectedCategory.name];
		return subcats ?? [];
	});

	function getLocalFilteredWorkflows(): ITemplatesWorkflow[] {
		const selectedCategory = LOCAL_CATEGORIES.find(
			(c) => String(c.id) === selectedCategoryId.value,
		);

		let filtered = selectedCategory
			? typedLocalData.workflows.filter((w) => w.category === selectedCategory.name)
			: typedLocalData.workflows;

		if (selectedSubcategory.value) {
			filtered = filtered.filter((w) => w.subcategory === selectedSubcategory.value);
		}
		...
		if (!selectedCategory && !selectedSubcategory.value) {
			return [
				...(FEATURED_DEFAULTS as unknown as ITemplatesWorkflow[]),
				...(sorted as unknown as ITemplatesWorkflow[]),
			];
		}

		return sorted as unknown as ITemplatesWorkflow[];
	}

	function fetchLocalWorkflows() {
		const filtered = getLocalFilteredWorkflows();
		totalWorkflows.value = filtered.length;

		const start = (currentPage.value - 1) * PAGE_SIZE;
		workflows.value = filtered.slice(start, start + PAGE_SIZE);
	}
	...
	async function initialize() {
		await Promise.all([fetchCategories(), fetchWorkflows()]);
	}
